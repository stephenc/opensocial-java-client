...
public class GroupsExample {
  ...
  public static void main(String[] args) {
    ...
    try {
      ...
      // Get groups request
      batch.addRequest(c.getGroupsService().get(params), "createNotificaion");

      // Supported Fields
      batch.addRequest(c.getGroupsService().getSupportedFields(), "supportedFields");
      batch.send(c);
      ...
      for(Object id : responses) {
          ...
          if(resp.getStatusCode() > 201) {
            ...
            if(id.toString().equals("supportedFields")) {
              ...
              List<OpenSocialGroup> groups = resp.getGroupCollection();
              OpenSocialGroup group;
              
              for(int i=0; i < groups.size(); i++) {
                group = groups.get(i);
                System.out.println(group.getField("id"));
                System.out.println(group.getField("title"));
              }
            }
          }
      }
